...
def combine_adata(adata1, adata2, image_id1, image_id2):

    adata1.obs['image_id'] = [image_id1] * adata1.n_obs
    adata2.obs['image_id'] = [image_id2] * adata2.n_obs

    combined_adata = ad.concat([adata1, adata2], index_unique=None)
...
def combine_ann_data(fe_data):
    ...
    vars_union = set().union(*[set(adata.var_names) for adata in all_adatas])
    missing_vars_list = [vars_union - set(adata.var_names) for adata in all_adatas]

    all_adatas = [
        ad.concat([adata, ad.AnnData(np.zeros((adata.n_obs, len(missing_vars))),
                                     var=pd.DataFrame(index=list(missing_vars)),
                                     obs=adata.obs)], axis=1)
        if missing_vars else adata
        for adata, missing_vars in zip(all_adatas, missing_vars_list)
    ]

    if len(all_adatas) > 0:
        combined_adata = ad.concat(all_adatas, index_unique=None, join='outer')
        combined_obs_data = pd.concat(all_obs_data, ignore_index=True)
        combined_adata.obs = combined_obs_data
        return combined_adata
    else:
        return None
